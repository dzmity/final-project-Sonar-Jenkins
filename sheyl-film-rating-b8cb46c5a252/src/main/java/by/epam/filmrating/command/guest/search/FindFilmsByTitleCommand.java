...
/**
 * The {@code FindFilmsByTitleCommand} class is participant in Command Pattern.
 * It is a concrete class extends the {@link ActionCommand}.
 * The class is responsible for searching {@link Film} objects by title and
 * displaying their in a tabular format and page mode.
 * @author Dmitry Rafalovich
 */
public class FindFilmsByTitleCommand implements ActionCommand {
    ...
    private static final String COMMAND = "find_films_by_title";
    ...
    public String execute(SessionRequestContent content) {
        ...
        String fieldTitle[] = content.getRequestParameters().get(SEARCH_WORD);

        if (fieldTitle == null) {
            LOG.error("'Search' field must be filled. Check 'required' parameter for " +
                    "this <input> tag in films.jsp.");
            return (String) content.getSessionAttributes().get(CURRENT_PAGE);
        }

        String[] words = fieldTitle[0].trim().split(FilmRatingRegEx.GREEDY_SPACES);
        ...
        String command = COMMAND + PARAMETER + title;
        ...
        try {
            ...
            LOG.error("Exception in FindFilmsByTitleCommand", e);
            return (String) content.getSessionAttributes().get(CURRENT_PAGE);
        }
